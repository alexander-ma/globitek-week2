<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    if (is_blank($state['code'])) {
      ...
      $errors[] = "Code must be less than 2 characters.";
  } elseif (!has_valid_code_format($state['code'])) {
      $errors[] = "Code must contain letters only.";

  }

    if (is_blank($state['country_id'])) {
      $errors[] = "Country ID cannot be blank.";
  } elseif (!has_valid_id_format($state['country_id'])) {
      $errors[] = "Country ID must contain numbers only.";
  ...
  function insert_state($state) {
    ...
    $sql .= "(name, code, country_id) VALUES (";
    $sql .= "'" . $state['name'] . "', ";
    $sql .= "'" . $state['code'] . "', ";
  ...
  function validate_user($user, $errors=array()) {
    ...
    } else if (!has_length($user['username'], array('max' => 255))) {
      $errors[] = "Username must be less than 255 characters.";
